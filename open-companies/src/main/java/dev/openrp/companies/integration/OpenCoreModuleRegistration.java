package dev.openrp.companies.integration;
...
/**
 * Optional, fully reflective registration of Open Companies as an OpenCore module, mirroring the
 * pattern other Open Roleplay modules use. Open Companies manages its own JavaPlugin lifecycle, so the
 * registered module's {@code onEnable}/{@code onDisable} are intentionally no-ops - registering simply
 * makes the module visible to OpenCore (e.g. in its module listing). The plugin runs perfectly well
 * standalone; {@link #register()} returns {@code false} when OpenCore is absent and nothing else
 * depends on it.
 */
public final class OpenCoreModuleRegistration {

    private static final String MODULE_ID = "open-companies";
    ...
    public boolean register() {
        ...
        try {
            ...
            plugin.getLogger().info("[OpenCompanies] Registered in OpenCore.");
            return true;
        } catch (ReflectiveOperationException | LinkageError error) {
            plugin.getLogger().warning("[OpenCompanies] OpenCore found, but module registration failed: "
                    + rootMessage(error));
            reset();
            return false;
        }
    ...
    public void unregister() {
        ...
        try {
            ...
            plugin.getLogger().warning("[OpenCompanies] Unregister from OpenCore failed: " + rootMessage(error));
        } finally {
            reset();
        }
    }
    ...
    private InvocationHandler moduleHandler() {
        return (proxy, method, args) -> switch (method.getName()) {
            ...
            case "toString" -> "OpenCompaniesModule";
