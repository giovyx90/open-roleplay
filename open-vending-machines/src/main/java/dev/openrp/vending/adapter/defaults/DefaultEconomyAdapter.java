...
/**
 * In-memory demo economy. Player wallets start at a configurable balance the first time they are
 * seen; named (company) accounts start empty. Balances are not persisted - this exists so the
 * plugin is fully playable out of the box. Replace it with a real adapter (e.g. Vault) in production.
 */
public final class DefaultEconomyAdapter implements EconomyAdapter {
    ...
    public DefaultEconomyAdapter(double startingBalance) {
    ...
    public boolean withdraw(OfflinePlayer player, String account, double amount) {
        ...
        // Atomic read-modify-write: the same player buying from two machines at once can't have both
        // withdrawals pass the balance check and over-withdraw. A never-seen wallet holds the starting balance.
        boolean[] ok = {false};
