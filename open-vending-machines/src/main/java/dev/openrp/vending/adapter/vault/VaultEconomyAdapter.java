package dev.openrp.vending.adapter.vault;
 ...
 * plugin keeps <em>zero</em> compile-time or mandatory runtime dependency on Vault (same approach
 ...
 *
 * <p>Vault models a single balance per player, so the {@code account} argument is ignored for
 * player methods. Named (company) accounts map to Vault <em>banks</em> when the provider supports
 * them; otherwise those operations return {@code false}/0 and the core pays the player directly.</p>
 */
public final class VaultEconomyAdapter implements EconomyAdapter {
    ...
    private <T> T warnAndReturn(ReflectiveOperationException exception, T fallback) {
        logger.warning("[OpenVendingMachines] Vault economy call failed: " + exception.getMessage());
        return fallback;
    }
}
