package dev.openrp.fdo.integration;
...
/**
 * Optional, fully reflective registration of Open FDO as an OpenCore module, mirroring the pattern
 * the other Open Roleplay modules use. Open FDO manages its own JavaPlugin lifecycle, so the
...
public final class OpenCoreModuleRegistration {
    ...
    private static final String MODULE_ID = "open-fdo";
    ...
    public boolean register() {
        ...
        try {
            ...
            plugin.getLogger().info("[OpenFDO] Registered in OpenCore.");
            return true;
        } catch (ReflectiveOperationException | LinkageError error) {
            plugin.getLogger().warning("[OpenFDO] OpenCore found, but module registration failed: "
                    + rootMessage(error));
            reset();
            return false;
        }
    ...
    public void unregister() {
        ...
        try {
            ...
            plugin.getLogger().warning("[OpenFDO] Unregister from OpenCore failed: " + rootMessage(error));
        } finally {
            reset();
        }
    }
