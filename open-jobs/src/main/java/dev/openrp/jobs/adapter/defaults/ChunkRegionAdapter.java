 ...
 * from the chunk a location sits in ({@code world@chunkX,chunkZ}) and treats every tag as satisfied, so
 * work locations are usable out of the box. It reports {@link #available()} as {@code false}: features
 * gated behind {@code general.require_region_backend} stay disabled until a real {@code RegionAdapter}
 * (e.g. a WorldGuard bridge) is registered.
 */
