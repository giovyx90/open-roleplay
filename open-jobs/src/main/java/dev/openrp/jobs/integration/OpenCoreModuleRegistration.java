package dev.openrp.jobs.integration;
...
/**
 * Optional, fully reflective registration of Open Jobs as an OpenCore module, mirroring the pattern the
 * other Open Roleplay modules use. Open Jobs manages its own JavaPlugin lifecycle, so the registered
 * module's callbacks are no-ops - registering just makes the module visible to OpenCore. The plugin
 * runs perfectly standalone; {@link #register()} returns {@code false} when OpenCore is absent.
 */
public final class OpenCoreModuleRegistration {

    private static final String MODULE_ID = "open-jobs";
    ...
    public boolean register() {
        ...
        try {
            ...
            plugin.getLogger().info("[OpenJobs] Registered in OpenCore.");
            return true;
        } catch (ReflectiveOperationException | LinkageError error) {
            plugin.getLogger().warning("[OpenJobs] OpenCore found, but module registration failed: "
                    + rootMessage(error));
            reset();
            return false;
        }
    ...
    public void unregister() {
        ...
        try {
            ...
            plugin.getLogger().warning("[OpenJobs] Unregister from OpenCore failed: " + rootMessage(error));
        } finally {
            reset();
        }
    }
