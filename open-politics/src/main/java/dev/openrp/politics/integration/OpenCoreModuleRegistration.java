package dev.openrp.politics.integration;
...
/**
 * Optional, fully reflective registration of Open Politics as an OpenCore module, mirroring the pattern
 * the other Open Roleplay modules use. Open Politics manages its own JavaPlugin lifecycle, so the
 * registered module's callbacks are no-ops - registering just makes the module visible to OpenCore. The
 * plugin runs perfectly standalone; {@link #register()} returns {@code false} when OpenCore is absent
 * and nothing else depends on it.
 */
public final class OpenCoreModuleRegistration {

    private static final String MODULE_ID = "open-politics";
    ...
    public boolean register() {
        ...
        try {
            ...
            plugin.getLogger().info("[OpenPolitics] Registered in OpenCore.");
            return true;
        } catch (ReflectiveOperationException | LinkageError error) {
            plugin.getLogger().warning("[OpenPolitics] OpenCore found, but module registration failed: "
                    + rootMessage(error));
            reset();
            return false;
        }
    ...
    public void unregister() {
        ...
        try {
            ...
            plugin.getLogger().warning("[OpenPolitics] Unregister from OpenCore failed: " + rootMessage(error));
        } finally {
            reset();
        }
    }
    ...
    private InvocationHandler moduleHandler() {
        return (proxy, method, args) -> switch (method.getName()) {
            ...
            case "toString" -> "OpenPoliticsModule";
