...
- `OpenJobs`: plugin standalone iniziale e compilabile, adapter-first e neutro rispetto all'ambientazione; i lavori base individuali e informali (vai, lavori, guadagni) accanto alle aziende strutturate di OpenCompanies. Lavori, tipi di location e scala di anzianita' sono tutti definiti in config. Filosofia RP First: paga l'attivita' realmente svolta nella sessione (blocchi estratti, pesci pescati, item trasformati), non i minuti trascorsi nella region. Tre modelli di paga (a produzione, a sessione, a consegna) piu' lavori trasformativi pagati per crafting; progressione per sessioni reali nel tempo con decadimento opzionale; licenze professionali revocabili. Economia, aziende, identita' e regioni passano da adapter opzionali; assenti o non reali, la feature collegata degrada in silenzio. Espone `OpenJobsApi`.
- `OpenPolitics`: plugin standalone iniziale e compilabile, adapter-first e neutro rispetto all'ambientazione; cariche, governi, elezioni, nomine, ereditarieta', conquiste, atti firmati e registro pubblico delle leggi sono tutti definiti in config. Filosofia RP First: il plugin certifica chi aveva l'autorita' di decidere e conserva l'atto, ma non esegue mai le conseguenze della decisione. Iter legislativo con voto collegiale, quorum, veto e promulgazione; archivio storico per leggi abrogate. Economia, aziende, identita', regioni e autorita' passano da adapter opzionali; assenti o non reali, la feature collegata degrada in silenzio. Espone `OpenPoliticsApi`.
...
open-roleplay/
  ...
  open-jobs/
    src/main/java/dev/openrp/jobs/
    src/main/resources/
    src/test/java/dev/openrp/jobs/
    examples/                 # realistico-it/ e fantasy/
  open-politics/
    src/main/java/dev/openrp/politics/
    src/main/resources/
    src/test/java/dev/openrp/politics/
    examples/                 # fantasy/ e oligarchia/
